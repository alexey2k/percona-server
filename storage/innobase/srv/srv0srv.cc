...
/*==================*/
{
	...
	ut_a(srv_force_recovery < SRV_FORCE_NO_BACKGROUND);
        ...
        if (srv_trace_purging>2) {
          fprintf(stderr,
          "worker thread %lu starts working\n",
           os_thread_pf(os_thread_get_curr_id()));
           start_ms = ut_time_ms();
        }
	...
	if (thr != NULL) {
        ...
        if (srv_trace_purging>2) {
                fprintf(stderr,
                        ...
                        os_thread_pf(os_thread_get_curr_id()), thr,
                        ut_time_ms() - start_ms,os_atomic_inc_ulint(&purge_sys->bh_mutex, &purge_sys->n_completed, 0));
        }
						...
						required by os_thread_create */
{
	...
	do {
		...
		os_event_wait(slot->event);
                ...
                if (srv_trace_purging>2) {
                        fprintf(stderr,
                                "purge worker %lu woke up after %lu ms\n",
                                (ulong) os_thread_pf(os_thread_get_curr_id()),
                                (ulong) (ut_time_ms() - start_tstamp));
                }
			...
			undo_trunc_freq);
                ...
                if (srv_trace_purging>0) {
                        fprintf(stderr,
                                ...
                                "using %lu threads, batch size %lu, count: %ld, rseg_len:%ld, trx_sys_hist_len:%ld, truncate_cond:%ld\n",
                                n_threads, n_use_threads,
                                srv_purge_batch_size, count,
		...
		*n_total_purged += n_pages_purged;
                ...
                if (srv_trace_purging>0) {
                        fprintf(stderr,
                                ...
                                "using:%lu, batch size:%lu, prgd:%ld, prgd_total:%ld, hist_len_old:%ld, hist_len:%ld, time:%ld\n",
                                (count % TRX_SYS_N_RSEGS),
                                n_threads, n_use_threads,
                                srv_purge_batch_size,
                                n_pages_purged,*n_total_purged,
                                rseg_history_len,trx_sys->rseg_history_len,ut_time_ms()-loop_time_ms);
                }
		...
		if (stop) {
                        ...
                        if (srv_trace_purging>0) {
                                fprintf(stderr,
                                        ...
                                        "suspended for %lu us, "
                                        "dirty hist len %lu\n",
                                        ut_time_us(NULL) - suspend_tstamp,
                                        trx_sys->rseg_history_len);
                        }
			ret = 0;
	...
	do {
		...
		if (srv_shutdown_state == SRV_SHUTDOWN_NONE
			...
			|| n_total_purged == 0)) {
                       ...
                       if (srv_trace_purging>0) {
                                fprintf(stderr,
                                        "coordinator: last purge purged %lu\n",
                                        n_total_purged);
                        }
